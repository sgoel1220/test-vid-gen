    ...
    async def create_pod(self, spec: GpuPodSpec, idempotency_key: str) -> GpuPod:
        ...
        if existing:
            ...
            if existing.status == GpuPodStatus.CREATING:
                ...
                if age_sec is not None and age_sec >= pull_stuck_timeout_sec:
                    log.warning(
                        "create_pod: vastai instance %s (label=%s) stuck CREATING for %ds — terminating",
                        existing.id,
                        idempotency_key,
                        int(age_sec),
                    )
                ...
                # Unknown age or within timeout — let wait_for_ready handle stuck detection
                return existing
